...
class PostCursor {
	...
	public function get_where() {
		...
		if ( ! empty( $orderby ) && is_array( $orderby ) ) {
			...
			foreach ( $orderby as $by => $order ) {
				$where .= $this->compare_with_field( $by, $order );
			}
			return $where;
		} else if ( ! empty( $orderby ) && is_string( $orderby ) ) {
			$order = ! empty( $this->query->query_vars['order'] ) ? $this->query->query_vars['order'] : 'DESC' ;
			return $this->compare_with_field( $orderby, $order );
		}

		if ( ! empty( $this->cursor_post->post_date ) ) {
	...
	/**
	 * Get AND operator field or meta field based comparison
	 */
	private function compare_with_field( $by, $order ) {
		global $wpdb;
		...
		// XXX Bad post field and meta field. Should be we just throw here?
		return '';
	}
